...
Env:
  RPC_URL - URL to RPC (give each parallel worker its own Envio endpoint/API token)
  PG_DSN - postgres
...
Usage:
  python envio_rpc.py --start 18000000 --end 18005000
  python envio_rpc.py --start 18000000 --end 18005000 --chunk 500 --block-batch 100
  python envio_rpc.py --start 18000000 --yaml config/stablecoins_detailed.yaml
"""
...
CREATE TABLE IF NOT EXISTS bep20_transfer (
  ...
  FOREIGN KEY (block_number, tx_index) REFERENCES transaction(block_number, tx_index)
);
...
CREATE TABLE IF NOT EXISTS token_event (
  ...
  a0_id          BIGINT REFERENCES address(id),
  a1_id          BIGINT REFERENCES address(id),
  value          NUMERIC(78,0),
  PRIMARY KEY (block_number, tx_index, log_index),
  FOREIGN KEY (block_number, tx_index) REFERENCES transaction(block_number, tx_index)
);
...
MIGRATION_SQL = """
ALTER TABLE transaction ADD COLUMN IF NOT EXISTS value NUMERIC(78,0) NOT NULL DEFAULT 0;
ALTER TABLE transaction ADD COLUMN IF NOT EXISTS gas_price NUMERIC(78,0);
ALTER TABLE transaction ADD COLUMN IF NOT EXISTS gas_used BIGINT;
ALTER TABLE transaction ADD COLUMN IF NOT EXISTS effective_gas_price NUMERIC(78,0);
ALTER TABLE transaction ADD COLUMN IF NOT EXISTS success BOOLEAN;
"""
...
def upsert_addresses(
    ...
    addrs: Iterable[Optional[bytes]],
    max_retries: int = 5,
) -> Dict[bytes, int]:
    """
    Insert addresses and return {address_bytes: id}.

    The addresses are sorted before insertion so that multiple scraper
    processes acquire PostgreSQL row/index locks in a consistent order.
    Deadlocks/serialization failures are retried defensively.
    """
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            wait = min(0.5 * attempt, 3.0)
            print(
                f"Address upsert hit DB contention "
                f"(attempt {attempt}/{max_retries}): {e}. "
                f"Retrying in {wait:.1f}s..."
            )
            time.sleep(wait)

    raise RuntimeError("Address upsert retry loop exited unexpectedly")
...
def upsert_tokens(
    ...
    """
    Insert token addresses and return {token_address_bytes: id}.

    Sorting gives concurrent scraper workers a deterministic insertion order.
    Deadlocks/serialization failures are retried defensively.
    """
    token_set: Set[bytes] = {a for a in addrs if a is not None}
    if not token_set:
        return {}
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            wait = min(0.5 * attempt, 3.0)
            print(
                f"Token upsert hit DB contention "
                f"(attempt {attempt}/{max_retries}): {e}. "
                f"Retrying in {wait:.1f}s..."
            )
            time.sleep(wait)

    raise RuntimeError("Token upsert retry loop exited unexpectedly")
...
def insert_blocks_and_txs(
    ...
    with conn.cursor() as cur:
        if block_rows:
            cur.executemany(
                """
                INSERT INTO block(block_number, ts)
...
def fetch_block_receipts_adaptive(rpc: RpcClient, block_nums: List[int], start_batch: int) -> List[List[dict]]:
    ...
    while i < len(block_nums):
        chunk = block_nums[i:i + batch]
        try:
            print(f"Fetching receipts for blocks {chunk[0]} - {chunk[-1]} with batch={batch}")
            res = rpc.eth_get_block_receipts_batch(chunk)
            out.extend(res)
            i += len(chunk)
        except RuntimeError as e:
            msg = str(e).lower()
            if (
                ...
                if batch > 1:
                    batch = max(1, batch // 2)
                    print(f"Receipts fetch failed ({e}); reducing batch size to {batch}")
                    continue

                # batch already 1 -> retry single block once more after short sleep
                print(f"Single-block receipts fetch failed ({e}); retrying once")
                time.sleep(2)
                res = rpc.eth_get_block_receipts_batch(chunk)
                out.extend(res)
                i += len(chunk)
